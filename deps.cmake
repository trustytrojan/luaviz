include(FetchContent)

# Option to allow using a local checkout of libaudioviz instead of fetching
# If empty, deps will be fetched from the git URL below. If set, the path
# must point at a directory containing libaudioviz's CMakeLists.txt.
option(LUAVIZ_LOCAL_LIBAUDIOVIZ_PATH "Path to local libaudioviz directory; if set, uses add_subdirectory" "")
option(LUAVIZ_LIBAUDIOVIZ_GIT_TAG "Git tag/branch to checkout for libaudioviz when fetching" "main")

set(AUDIOVIZ_BUILD_TTVIZ OFF CACHE BOOL "" FORCE)
set(AUDIOVIZ_BUILD_EXAMPLES OFF CACHE BOOL "" FORCE)
set(AUDIOVIZ_USE_IMGUI OFF CACHE BOOL "" FORCE)

# Make libaudioviz available either from a local path or by fetching the
# repository (default: ${LUAVIZ_LIBAUDIOVIZ_GIT_URL} @ ${LUAVIZ_LIBAUDIOVIZ_GIT_TAG}).
if(LUAVIZ_LOCAL_LIBAUDIOVIZ_PATH)
	# Resolve to an absolute path so we can correctly determine whether the
	# source is outside the current tree and provide a dedicated binary-dir.
	get_filename_component(_audioviz_src "${LUAVIZ_LOCAL_LIBAUDIOVIZ_PATH}" ABSOLUTE)
	if(EXISTS "${_audioviz_src}/CMakeLists.txt")
		set(_audioviz_bin "${_audioviz_src}/build")
		message(STATUS "Using local libaudioviz at ${_audioviz_src}; binary dir=${_audioviz_bin}")
		add_subdirectory(${_audioviz_src} ${_audioviz_bin} EXCLUDE_FROM_ALL)
	else()
		message(FATAL_ERROR "LUAVIZ_LOCAL_LIBAUDIOVIZ_PATH is set but ${_audioviz_src}/CMakeLists.txt was not found")
	endif()
else()
	message(STATUS "Fetching libaudioviz from GitHub (tag: ${LUAVIZ_LIBAUDIOVIZ_GIT_TAG})")
	FetchContent_Declare(audioviz URL "https://github.com/trustytrojan/audioviz/archive/${LUAVIZ_LIBAUDIOVIZ_GIT_TAG}.tar.gz")
	FetchContent_MakeAvailable(audioviz)
endif()

target_link_libraries(luaviz PUBLIC audioviz)

if(LUAVIZ_BUILD_LUA)
	message("fetching lua source...")
	set(LUA_ENABLE_SHARED OFF)
	set(LUA_ENABLE_TESTING OFF)
	set(LUA_BUILD_BINARY OFF)
	set(LUA_BUILD_COMPILER OFF)
	FetchContent_Declare(lua URL https://github.com/walterschell/Lua/archive/master.tar.gz)
	FetchContent_MakeAvailable(lua)
	target_link_libraries(luaviz PUBLIC Lua::Library)
else()
	message("using system lua installation...")
	if(WIN32)
		# usual place that lua is installed (via winget or other method)
		set(LUA_PATH "$ENV{LocalAppData}/Programs/Lua")

		if(NOT IS_DIRECTORY "${LUA_PATH}")
			message(FATAL_ERROR "lua is not installed! install it or pass -DLUAVIZ_BUILD_LUA=1 to cmake")
		endif()

		if(CMAKE_SYSTEM_PROCESSOR MATCHES "x86_64|AMD64")
			set(LUA_LIB_NAME lua54) # this will need to be updated once lua 5.5 releases
			find_library(LUA_LIB ${LUA_LIB_NAME} PATHS "${LUA_PATH}/lib" REQUIRED)
			find_file(LUA_HEADER "lua.h" PATHS "${LUA_PATH}/include" REQUIRED)

			target_include_directories(luaviz PUBLIC ${LUA_PATH}/include)
			target_link_directories(luaviz PUBLIC ${LUA_PATH}/lib)
			target_link_libraries(luaviz PUBLIC ${LUA_LIB_NAME})
		else()
			message(FATAL_ERROR "there are no lua binaries for your architecture. pass -DLUAVIZ_BUILD_LUA=1 to cmake")
		endif()
	else()
		# for cross-distro compatibility since lua doesn't provide cmake support
		find_package(PkgConfig REQUIRED)
		pkg_search_module(LUA REQUIRED lua)
		if(NOT LUA_FOUND)
			message(FATAL_ERROR "lua not found! (pkg-config could not find it)")
		endif()
		message(STATUS "Found Lua ${LUA_VERSION}!")
		include_directories(${LUA_INCLUDE_DIRS})
		target_link_libraries(luaviz PUBLIC ${LUA_LIBRARIES})
	endif()
endif()

FetchContent_Declare(sol2 URL https://github.com/ThePhD/sol2/archive/main.tar.gz)
FetchContent_MakeAvailable(sol2)
target_link_libraries(luaviz PUBLIC sol2)
