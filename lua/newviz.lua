...
sa = luaviz.StereoAnalyzer.new(samplerate, FFT_SIZE)
bp = luaviz.BinPacker.new()
ip = luaviz.Interpolator.new()
...
-- sanity check
assert(lbsd:get_bar_count() == rbsd:get_bar_count(), 'bar counts are not the same!')
...
particles_layer:set_orig_cb(function(orig_rt)
	...
	sa:right():compute_amplitudes(fa)

	-- FIX THIS IN LIBAUDIOVIZ: SHAKE::SETPARAMETERS NEEDS TO TAKE A MULTICHANNELAUDIOANALYZER
	luaviz.Shake_setParameters(
		sa:left(),
		0, -- from_hz
		250, -- to_hz
		15 -- multiplier
...
spectrum_layer:set_orig_cb(function(orig_rt)
	...
	lbsd:update(fa, sa:left(), bp, ip)
	rbsd:update(fa, sa:right(), bp, ip)
